...
// (siehe @jm/audio index.js bundledBinDir):
...
//   $env:PORTAUDIO_DIR=…; npm run rebuild:native -w @jm/recorder).
...
if (!existsSync(addon)) {
  throw new Error(
    `[bundle-audio] jm_audio.node nicht gefunden: ${addon}\n` +
      '  Erst bauen: $env:PORTAUDIO_DIR="<…>"; npm run rebuild:native -w @jm/recorder',
  );
}
