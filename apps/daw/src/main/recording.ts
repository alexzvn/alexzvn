...
import { secToUs } from '@shared/project';
import type {
  ArmInput,
  AudioDevice,
  Levels,
...
// Lazy-Load des nativen Addons: die DAW startet auch ohne gebautes @jm/audio
// (Import/Schnitt/Mix/Export laufen über Web Audio + FFmpeg). Erst beim ersten
// Geräte-/Aufnahme-Zugriff geladen. Muster aus JM Recorder.
...
function emitState(): void {
  send('rec:state', { ...state } satisfies RecorderState);
}
...
function onFrames(planar: Float32Array, channels: number, frames: number): void {
  ...
  const now = Date.now();
  if (now - lastEmit >= 50) {
    send('rec:levels', { peaks: peaks.slice() } satisfies Levels);
...
export function startRecording(input: StartRecordInput): OpResult {
  ...
  try {
    ...
    const filePath = path.join(recordingsDir(), `${base}.wav`);
...
export function stopRecording(): RecordResult {
  ...
  try {
    ...
    return {
      ok: true,
      filePath,
      durationUs: secToUs(res.durationSec),
