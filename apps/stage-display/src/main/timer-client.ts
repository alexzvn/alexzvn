...
/** Minimaler Blick auf den vom JM Timer gebroadcasteten SyncedState. */
...
/** socket.io-Client auf den JM-Timer-Server (Port 7777). Reconnect via socket.io. */
export class TimerClient {
  ...
  connect(host: string, port: number): void {
    ...
    socket.on('connect_error', () => {
      /* bleibt offline; socket.io versucht weiter */
    });
    this.socket = socket;
  }
